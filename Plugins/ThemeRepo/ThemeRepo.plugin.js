//META{"name":"ThemeRepo","website":"https://github.com/ShasWHITE/BetterDiscordAddons-master/tree/master/BetterDiscordAddons-master/Plugins/ThemeRepo","source":"https://raw.githubusercontent.com/mwittrien/BetterDiscordAddons/master/Plugins/ThemeRepo/ThemeRepo.plugin.js"}*//
...
class ThemeRepo {
	...
	initConstructor () {
		...
		this.frameMarkup = 
			`<iframe class="discordPreview" src="https://ShasWHITE.github.io/BetterDiscordAddons/Plugins/ThemeRepo/res/DiscordPreview.html"></iframe>`;
	...
	openThemeRepoModal (options = {}) {
		...
		var keyPressed = e => {
			if (e.which == 17) {
				...
				if (toggle) BDFDB.DOMUtils.toggle(themeRepoModal);
			}
		...
		var createSelectChoice = index => {
			...
			return `<div class="${BDFDB.disCNS.title + BDFDB.disCNS.medium + BDFDB.disCNS.primary + BDFDB.disCNS.weightnormal + BDFDB.disCN.cursorpointer}" style="flex: 1 1 auto;"><strong>${BDFDB.StringUtils.htmlEscape(theme.name)}</strong> by ${BDFDB.StringUtils.htmlEscape(theme.author)}</div>`;
		};

		var saveSelectChoice = (selectWrap, type, index) => {
			let theme = this.loadedThemes[this.generatorThemes[index-1]] || {name: "-----", author: "-----"};
			selectWrap.querySelector(BDFDB.dotCN.title).innerHTML = `<strong>${BDFDB.StringUtils.htmlEscape(theme.name)}</strong> by ${BDFDB.StringUtils.htmlEscape(theme.author)}`;
			themeRepoModal.querySelectorAll(".previewCheckbox").forEach(checkbox => {
				checkbox.checked = false;
				BDFDB.DOMUtils.removeClass(checkbox.parentElement, BDFDB.disCN.switchvaluechecked);
				BDFDB.DOMUtils.addClass(checkbox.parentElement, BDFDB.disCN.switchvalueunchecked);
			});
			let container = themeRepoModal.querySelector(".variables");
			BDFDB.DOMUtils.remove(themeRepoModal.querySelectorAll(".varcontainer"));
		...
		var frame = BDFDB.DOMUtils.create(this.frameMarkup);
		var themeRepoModal = BDFDB.DOMUtils.create(this.themeRepoModalMarkup);
		...
		themefixerinput.addEventListener("change", e => {
			BDFDB.LibraryRequires.request("https://ShasWHITE.github.io/BetterDiscordAddons/Plugins/ThemeRepo/res/ThemeFixer.css", (error, response, body) => {
		...
		themeRepoModal.querySelector("#download-themefixer").addEventListener("click", e => {
			BDFDB.LibraryRequires.request("https://ShasWHITE.github.io/BetterDiscordAddons/Plugins/ThemeRepo/res/ThemeFixer.css", (error, response, body) => {
				this.createThemeFile("ThemeFixer.theme.css", `//META{"name":"ThemeFixer","description":"ThemeFixerCSS for transparent themes","author":"ShasWHITE","version":"1.0.2"}*//\n\n` + this.createFixerCSS(body));
			});
		});
	...
	createGeneratorVars (frame, container, data) {
		...
		var maxwidth = BDFDB.DOMUtils.getRects(container).width - 80;
			...
			if (varvalue) {
				...
				if (iscolor || iscomp) {
					...
					swatch.addEventListener("click", e => {
						BDFDB.ColorUtils.openPicker(varcontainer, e.currentTarget, color, {gradient:false, comp:iscomp, alpha:iscolor, callback:updatePreview});
					});
				}
	...
	addEntry (frame, themeRepoModal, container, data) {
		...
		let entry = BDFDB.DOMUtils.create(this.themeEntryMarkup);
	...
	loadThemes () {
		...
		BDFDB.LibraryRequires.request("https://ShasWHITE.github.io/BetterDiscordAddons/Plugins/ThemeRepo/res/ThemeList.txt", (error, response, body) => {
			if (!error && body) {
				...
				BDFDB.LibraryRequires.request("https://github.com/NFLD99/Better-Discord", (error2, response2, body2) => {
					...
					getThemeInfo(() => {
						...
						BDFDB.LibraryRequires.request("https://ShasWHITE.github.io/BetterDiscordAddons/Plugins/ThemeRepo/res/GeneratorList.txt", (error3, response3, body3) => {
	...
	checkForNewThemes () {
		BDFDB.LibraryRequires.request("https://ShasWHITE.github.io/BetterDiscordAddons/Plugins/ThemeRepo/res/ThemeList.txt", (error, response, result) => {
